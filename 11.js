// var s = prompt("");
// var num = s.split(';').map(item => Number(item) ** 2);
// console.log(num);
// var result = num.find(item => (item > 5 && item % 2 == 0));
// console.log(result);
var favorites = ['Nguyen Ngoc Huyen', 'money', 'villa'];
var info =
{
    name: 'MindX',
    age: 19
...
info.address = "Ha Noi";
info.age = 20;
// for (var key in info) console.log(key, ": ", info[key]);
// log("\n");
function log(text) {
    console.log(text);
}
...
//     return a + b + c;
// }
// if (sum3Numbers(a, b, c) > 20) log('......<option>');
// const minh = () => {
//     if (a + b + c > 20) log('......<option>');
// }
// minh();
// log(sum3Numbers(1,'1',1));
// function sum3NumbersWithInput()
// {
//     return Number(a)+Number(b)+Number(c);
// }
// log(sum3NumbersWithInput());
// setInterval(() => log('Nguyen Ngoc Huyen'), 2000);
// const sum = (a, b) => 
// {
//     console.log(`%c${a}`, "color : red");
//     console.log(`%c${b}`, "color : yellow");
// };
var users = 
[
    {
...
var n = Number(prompt("Enter an ID"));
for (const obj of users)
    if (obj.id == n) log(obj);
var keyword = prompt("Enter a name");
for (const obj of users)
    if ((obj.first_name + obj.last_name).includes(keyword)) log(obj.email);
log("age > 50: " + users.filter(obj => obj.age > 50).length);
log("is_married == false: " + users.filter(obj => !obj.is_married).length);
log("is_married == true: " + users.filter(obj => obj.is_married).length);
var result = users.map(obj => obj.job);
var finalResult = [];
for (const career of result)
    if (!finalResult.includes(career)) finalResult.push(career);
log(finalResult);
