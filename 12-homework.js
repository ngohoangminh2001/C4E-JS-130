...
var choice = Number(prompt("Enter your choice:\n0. Do nothing\n1. Sign in\n2. Register\n3. Display user list\n4. Display post list\n5. Display 1 post in detail\n6. Find posts by user"));
while (choice != 0 && choice != 1 && choice != 2 && choice != 3 && choice != 4 && choice != 5 && choice != 6)
    choice = Number(prompt("Invalid choice. Please enter again.\n0. Do nothing\n1. Sign in\n2. Register\n3. Display user list\n4. Display post list\n5. Display 1 post in detail\n6. Find posts by user"));
if (choice == 1) {
    var email = prompt("Email");
    while (email == "") email = prompt("Please enter your email");
    while (!email.includes("@") || !email.includes(".")) email = prompt("Invalid email. Please enter again");
    var found = users.find(obj => obj.email == email);
    console.log(found);
    if (found == null) alert("This email is not registered as an user.");
    else {
        var password = prompt("Password");
        while (password == "") password = prompt("Please enter your password");
        if (found.password != password) alert("Wrong password");
        else alert("Hello " + found.first_name + " " + found.last_name + "!");
    }
}
else if (choice == 2) {
    var email = prompt("Enter your email");
    while (email == "" || !email.includes("@") || !email.includes(".")) email = prompt("Please enter a valid email");
    var existed = users.find(obj => obj.email == email);
    if (existed != null) alert("This email has been registered");
    else {
        var password = prompt("Enter your password");
        while (password == "") password = prompt("Please enter a valid password");
        var first_name = prompt("Enter your first name");
        var last_name = prompt("Enter your last name");
        users.push({ id: (users.length + 1), first_name: first_name, last_name: last_name, email: email, password: password });
        alert("Create account successfully!");
        console.log(users[users.length - 1]);
    }
}
else if (choice == 3) {
    var keyword = prompt("Enter a name or an email");
    if (keyword == "") console.log(users);
    else {
        var found = users.filter(obj => (obj.email == keyword || keyword.includes(obj.first_name) || keyword.includes(obj.last_name)));
        if (found.length == 0) alert("The information you have entered is incorrect.");
        else console.log(found);
    }
}
else if (choice == 4) console.log(posts);
else if (choice == 5) {
    var postID = Number(prompt("Enter a post ID"));
    var found = posts.find(obj => obj.id == postID);
    if (found == null) alert("Invalid post ID");
    else console.log(found);
}
else {
    var email = prompt("Enter a user email");
    while (email == "") email = prompt("Please enter an email");
    while (!email.includes("@") || !email.includes(".")) email = prompt("Invalid email. Please enter again");
    var foundUser = users.find(obj => obj.email == email);
    while (foundUser == null) {
        email = prompt("Incorrect email. Please enter again");
        while (email == null) email = prompt("Please enter an email");
        while (!email.includes("@") || !email.includes(".")) email = prompt("Invalid email. Please enter again");
        foundUser = users.find(obj => obj.email == email);
    }
    var foundPosts = posts.filter(obj => obj.user_id == foundUser.id);
    if (foundPosts.length == 0) alert("The user " + foundUser.id + " has not posted anything yet.");
    else console.log(foundPosts);
}